...
def sanitize(filepath):
    ...
    mask = df.index.isin(blocklist)
    df.loc[mask, "genotype"] = "AA"
    replaced = df[mask].index.tolist()
    
    print(f"Replaced {len(replaced)} flagged SNPs: {replaced}")
    return df

if __name__ == "__main__":
    df = sanitize("data/genome_James_Jones_v5_Full_20230726173828.txt")
    print(f"\nReplaced {len(df[df['genotype'] == 'AA'])} flagged SNPs total.")